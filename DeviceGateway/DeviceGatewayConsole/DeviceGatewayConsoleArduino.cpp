...
int main(int argc, char* argv[])
{
	...
	while(input)
	{
		...
		else if (input==2)
		{
			...
			cin >> configFile;
			ISensorDevices *sdevices = (ISensorDevices *) (new SensorDevicesArduino());//getSensorDevices();
		...
		else if (input==6)
		{
			...
			for (it = allSMdetails.begin(); it != allSMdetails.end(); it++)
			{
				...
				{
					cout << "Sensor " << it1 - mote.sensorsInfo.begin() << ": {" << (*it1).type << "," << (*it1).parentMoteID << "," << (*it1).siUnit << "," << (*it1).minValue << "," << (*it1).maxValue << "," << (*it1).accuracy << "," << (*it1).offset <<  "}\n";	
				}
				cout << "\n";								
			}
		}
