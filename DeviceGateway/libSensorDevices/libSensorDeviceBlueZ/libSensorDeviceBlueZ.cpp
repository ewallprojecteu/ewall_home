...
void process_data_entry(vector<measurement> *allMeas, vector<sensor> *sensors, string moteID,  DataEntry *data)
{
	...
	if (data != NULL && data->meta_data.size > 0 && data->meta_data.values != NULL) {
		...
		bool hasMeta = false;
		bool hasValue = false;
		string sensorname, sensorunit, sensormote;
		sensorType stype;
		...
		for (i = 0; i < data->meta_data.size; i++) {
			...
			if (meta != NULL && meta->name != NULL) {
				...
				int cadev = getSensorNameUnitMote(metavalue, sensorname, sensorunit, sensormote);
				if (cadev == 1)
				{
					stype = (sensorType)metavalue;
					printf("%s in %s\n", sensorname.c_str(), sensorunit.c_str());
					hasMeta = true;
				}
			}
		}
		...
		if (data->choice == SIMPLE_DATA_ENTRY) {
			...
			if (hasMeta && !hasValue)
			{
				...
				sensor newsens = sensor(stype, moteID, sensorunit, 0, 0, 0);
...
extern "C"
{
	...
	static CommunicationPlugin bt_plugin;
	static CommunicationPlugin usb_plugin;
	static CommunicationPlugin tcp_plugin;
	...
	static int plugin_pthread_connection_loop(Context *ctx)
	{
		...
		threadctx->connection_loop_thread = static_cast<pthread_t*>(malloc(sizeof(pthread_t)));
...
int SensorDevicesBlueZ::initializeDevice(vector<moteDetails> sensorMotes)
{
	...
	int opmode = DBUS_SERVER;//DBUS_SERVER;//AUTOTESTING;//TCP_SERVER;
	...
	if (tcpp_support) {
        	//plugin_network_tcp_setup(&tcp_plugin, 1, 6024);
