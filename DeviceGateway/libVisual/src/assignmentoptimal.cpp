...
void assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	for (i = 0; i < nOfColumns; i++) coveredColumns[i] = false;
	for (i = 0; i < nOfRows; i++) coveredRows[i] = false;
	...
	if (cost != NULL) computeassignmentcost(assignment, cost, distMatrixIn, nOfRows);
...
void computeassignmentcost(int *assignment, double *cost, double *distMatrix, int nOfRows)
{
	...
	for(row=0; row<nOfRows; row++)
	{
		...
		if(col >= 0)
		{
			...
			if(value < MAXABS_64F)
			{
				//cout << "Finite value of " << value << endl;
				*cost += value;
				//cout << "Cost is now " << *cost << endl;
			}
			else
			{
#ifdef ONE_INDEXING
				assignment[row] = 0;
				//cout << "Assignment for this row reset to 0" << endl;
#else
				assignment[row] = -1;
				//cout << "Assignment for this row reset to -1" << endl;
#endif
			}
#else
