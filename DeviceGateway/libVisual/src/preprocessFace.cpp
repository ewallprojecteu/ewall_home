...
const double DESIRED_LEFT_EYE_X = 0.2;  // nkat:Originally 0.16  // Controls how much of the face is visible after preprocessing.
const double DESIRED_LEFT_EYE_Y = 0.25;  // nkat:Originally 0.14
const double FACE_ELLIPSE_CY = 0.25;  // nkat: originally 0.40
const double FACE_ELLIPSE_W = 0.55;         // Should be atleast 0.5, nkat: originally 0.50
...
*/
    ...
    // For mcs.xml: Finds both eyes in roughly 80% of detected faces, also detects closed eyes.
    const float EYE_SX = 0.05f;
    const float EYE_SY = 0.15f;
    const float EYE_SW = 0.45f;
    const float EYE_SH = 0.40f;
...
*/
	...
	// Enlarge face images so that they have this width
	const int MIN_EYE_WIDTH = 48;
    ...
    // Enlarge the eye regions to detect eyes in small rectangles
    Mat inputImg;
    float scale = (float)MIN_EYE_WIDTH/widthX;
    if (scale > 1) {
        // Enlarge the image while keeping the same aspect ratio.
        int scaledHeight = cvRound(heightY * scale);
        resize(topLeftOfFace, topLeftOfFace, Size(MIN_EYE_WIDTH, scaledHeight));
        resize(topRightOfFace, topRightOfFace, Size(MIN_EYE_WIDTH, scaledHeight));
    }
    ...
    if (leftEyeRect.width > 0) {   // Check if the eye was detected.
		if (scale>1) //Adjust for scale
		{		
			leftEyeRect.x=cvRound(leftEyeRect.x/scale);
			leftEyeRect.y=cvRound(leftEyeRect.y/scale);
			leftEyeRect.width=cvRound(leftEyeRect.width/scale);
			leftEyeRect.height=cvRound(leftEyeRect.height/scale);
		}
    ...
    if (rightEyeRect.width > 0) { // Check if the eye was detected.
		if (scale>1) //Adjust for scale
		{		
			rightEyeRect.x=cvRound(rightEyeRect.x/scale);
			rightEyeRect.y=cvRound(rightEyeRect.y/scale);
			rightEyeRect.width=cvRound(rightEyeRect.width/scale);
			rightEyeRect.height=cvRound(rightEyeRect.height/scale);
		}
...
void normalizeIntensity(Mat &img, double desiredStd)
{
	vector<double> imgMean, imgStdev;
	meanStdDev(img,imgMean, imgStdev);
	Mat tmp;
	img.convertTo(tmp,CV_32F,1,-imgMean[0]);
	tmp=tmp/imgStdev[0]*desiredStd+128;
...
// Histogram Equalize seperately for the left and right sides of the face.
void equalizeLeftRightCenter(Mat &faceImg)
{
    // It is common that there is stronger light from one half of the face than the other. In that case,
    // if you simply did histogram equalization on the whole face then it would make one half dark and
    // one half bright. So we will do histogram equalization separately on each face half, so they will
    ...
    int w = faceImg.cols;
    int h = faceImg.rows;
	double desiredStd = 60;

    // 1) First, equalize the center half.
    Mat centerSide = faceImg(Rect(w/4,0, w/2,h)).clone();
    normalizeIntensity(centerSide, desiredStd);

    // 2) Equalize the left half and the right half of the face separately.
    int midX = w/2;
    Mat leftSide = faceImg(Rect(0,0, midX,h));
    Mat rightSide = faceImg(Rect(midX,0, w-midX,h));
    normalizeIntensity(leftSide, desiredStd);
    normalizeIntensity(rightSide, desiredStd);
    ...
    for (int y=0; y<h; y++) {
        for (int x=0; x<w; x++) {
            ...
            if (x < w/4) {          // Left 25%: just use the left face.
                ...
                int lv = leftSide.at<uchar>(y,x);
                int wv = centerSide.at<uchar>(y,x-w/4);
                // Blend more of the whole face as it moves further right along the face.
                float f = (x - w*1/4) / (float)(w*0.25f);
                v = cvRound((1.0f - f) * lv + (f) * wv);
            }
...
Mat getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, int doLeftAndRightSeparately, Rect *storeFaceRect, Point *storeLeftEye, Point *storeRightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    // Use square faces.
//    int desiredFaceHeight = desiredFaceWidth;
    int desiredFaceHeight = cvRound(1.125*desiredFaceWidth);
    ...
    if (faceRect.width > 0) {
        ...
        detectBothEyes(gray(faceRect), eyeCascade1, eyeCascade2, leftEye, rightEye, searchedLeftEye, searchedRightEye);

		//Reference eyes and search regions to full image
		leftEye.x+=faceRect.x; leftEye.y+=faceRect.y;
		rightEye.x+=faceRect.x; rightEye.y+=faceRect.y;
		if (searchedLeftEye)
		{
		...
		//Draw the results
			if (fabs(angle)>16)
			{
				//Mat dispImg=faceImg.clone();
				//circle(dispImg,leftEye,6,CV_RGB(255,0,0),2,CV_AA);
				//circle(dispImg,rightEye,6,CV_RGB(0,0,255),2,CV_AA);
				//cv::putText(dispImg,format("Angle: %.1f",angle),Point(10,20),FONT_HERSHEY_DUPLEX,0.7,CV_RGB(0,255,0),1,CV_AA);
				//imshow("Eyes",dispImg);
				//waitKey(1000);
				return Mat();
			}
            ...
            if (doLeftAndRightSeparately==0) {
                // Do it on the whole face.
                equalizeHist(warped, warped);
            }
            else if (doLeftAndRightSeparately==1) {
                // Do it seperately for the left and right sides of the face.
                equalizeLeftAndRightHalves(warped);
            }
            else {
                // Do our normalization for left, right and center sides of the face.
                equalizeLeftRightCenter(warped);
            }
            //imshow("equalized", warped);

			if (doLeftAndRightSeparately<3)
			{
				// Use the "Bilateral Filter" to reduce pixel noise by smoothing the image, but keeping the sharp edges in the face.
				Mat filtered = Mat(warped.size(), CV_8U);
				bilateralFilter(warped, filtered, 0, 20.0, 1.5);
