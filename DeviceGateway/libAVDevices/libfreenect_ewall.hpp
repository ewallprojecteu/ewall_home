	...
	class FreenectDevice : Noncopyable {
	  public:
			...
			: m_video_resolution(FREENECT_RESOLUTION_MEDIUM), m_depth_resolution(FREENECT_RESOLUTION_MEDIUM)
		{
            ...
            //TODO: Any formating for audio?? (KT)
			freenect_set_depth_callback(m_dev, freenect_depth_callback);
			freenect_set_video_callback(m_dev, freenect_video_callback);
            		freenect_set_audio_in_callback(m_dev, freenect_audio_callback);
		}
		virtual ~FreenectDevice() {
			if(freenect_close_device(m_dev) < 0){} //FN_WARNING("Device did not shutdown in a clean fashion");
		}
		void startAudio() {
			if(freenect_start_audio(m_dev) < 0) throw std::runtime_error("Cannot start audio callback");
		}
		void stopAudio() {
			if(freenect_stop_audio(m_dev) < 0) throw std::runtime_error("Cannot stop audio callback");
		}
		void startVideo() {
		...
		// Do not call directly even in child
        	virtual void AudioCallback(int num_samples, int32_t* mic1, int32_t* mic2, int32_t* mic3, int32_t* mic4,uint32_t timestamp) { }
	  ...
	  private:
		...
		static void freenect_audio_callback(freenect_device *dev, int num_samples,
                                            int32_t* mic1, int32_t* mic2, int32_t* mic3, int32_t* mic4,
                                            int16_t* cancelled, void *unknown) {
			FreenectDevice* device = static_cast<FreenectDevice*>(freenect_get_user(dev));
			device->AudioCallback( num_samples, mic1, mic2, mic3, mic4, 0);
		}
	};
	...
	class Freenect : Noncopyable {
	  ...
	  public:
		Freenect() : m_stop(false) {
			...
			// A change is made here to support audio - KT
			freenect_select_subdevices(m_ctx, static_cast<freenect_device_flags>(FREENECT_DEVICE_MOTOR | FREENECT_DEVICE_CAMERA | FREENECT_DEVICE_AUDIO));
